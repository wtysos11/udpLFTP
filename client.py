...
clientReceivePackNumMax = 50 #最多能够接受50个这样的数据包

s = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
s.bind(('127.0.0.1',9990))
jsonOptions = bytes(json.dumps({'filename':"test.txt","operation":"download"}),encoding='utf-8')
s.sendto(generateBitFromDict({"optLength":len(jsonOptions),"Options":jsonOptions,"RecvWindow":clientReceivePackMax}),('127.0.0.1',9999))
expectedSeqValue = 1
'''
GBN接受方逻辑
不断收包
...
while True:
    ...
    if packet.dict["FIN"] == b'1':#如果收到FIN包，则退出
        ...
        s.sendto(generateBitFromDict({"ACKvalue":expectedSeqValue,"ACK":b'1',"RecvWindow":clientReceivePackMax*clientReceivePackNumMax}),('127.0.0.1',int(addr[1])-1))
        expectedSeqValue += 1
    else:#收到了不对的包
        #print("Wrong data.",expectedSeqValue)
        s.sendto(generateBitFromDict({"ACKvalue":0,"ACK":b'1',"RecvWindow":clientReceivePackMax*clientReceivePackNumMax}),('127.0.0.1',int(addr[1])-1))
